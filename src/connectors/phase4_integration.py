...
from ..connectors.enhanced_common_crawl import EnhancedCommonCrawlConnector, CommonCrawlResult
from ..connectors.memento_temporal import MementoTemporalConnector, MementoTemporalResult
from ..connectors.ahmia_tor_connector import AhmiaTorConnector, AhmiaSearchResult
...
class Phase4ProcessingResult:
    ...
    # Connector results
    common_crawl_result: Optional[CommonCrawlResult]
    temporal_analysis_result: Optional[MementoTemporalResult]
    ahmia_result: Optional[AhmiaSearchResult]
    legal_apis_result: Optional[LegalSearchResult]
...
class Phase4Integrator:
    ...
    async def process_specialized_sources(self,
                                        query: str,
                                        search_parameters: Optional[Dict[str, Any]] = None) -> Phase4ProcessingResult:
        """
        ...
        try:
            ...
            logger.info(f"Connector success rate: {stability_metrics.get('overall_success_rate', 0):.1%}")
    ...
    async def _execute_connector_searches(self,
                                        query: str,
                                        search_parameters: Dict[str, Any]) -> Dict[str, SpecializedSourceResult]:
        ...
        for connector_name, result in connector_results.items():
            self._log_processing_step(f"{connector_name}_search", {
    ...
    async def _search_common_crawl(self, query: str, params: Dict[str, Any]) -> SpecializedSourceResult:
    ...
    async def _search_memento_temporal(self, query: str, params: Dict[str, Any]) -> SpecializedSourceResult:
    ...
    async def _search_ahmia_tor(self, query: str, params: Dict[str, Any]) -> SpecializedSourceResult:
    ...
    async def _search_legal_apis(self, query: str, params: Dict[str, Any]) -> SpecializedSourceResult:
    ...
    def _create_error_result(self, connector_name: str, error: Exception) -> SpecializedSourceResult:
    ...
    async def _perform_temporal_diff_analysis(self,
                                            connector_results: Dict[str, SpecializedSourceResult]) -> Dict[str, Any]:
        ...
        if temporal_result and temporal_result.success and temporal_result.result_data:
            ...
            if hasattr(temporal_data, 'temporal_diffs') and temporal_data.temporal_diffs:
                ...
                # Summarize changes
                significant_changes = [
                    diff for diff in temporal_data.temporal_diffs
                    if diff.significance_score >= 0.3
                ]

                diff_analysis["diff_summary"] = {
                    "total_diffs": len(temporal_data.temporal_diffs),
                    "significant_changes": len(significant_changes),
                    "avg_significance": sum(d.significance_score for d in temporal_data.temporal_diffs) / len(temporal_data.temporal_diffs),
    ...
    def _analyze_cross_source_consistency(self,
                                        connector_results: Dict[str, SpecializedSourceResult]) -> float:
        """Analýza consistency napříč sources"""

        successful_results = [
            result for result in connector_results.values()
    ...
    def _calculate_result_consistency(self,
                                    result_a: SpecializedSourceResult,
                                    result_b: SpecializedSourceResult) -> float:
    ...
    def _get_result_count(self, result_data: Any) -> int:
        ...
        try:
            ...
            elif hasattr(result_data, 'warc_records'):
                return len(result_data.warc_records)
            elif hasattr(result_data, 'legal_sources'):
                return len(result_data.legal_sources)
            elif hasattr(result_data, 'court_documents'):
    ...
    def _calculate_stability_metrics(self,
                                   connector_results: Dict[str, SpecializedSourceResult]) -> Dict[str, float]:
        ...
        overall_success_rate = successful_connectors / total_connectors if total_connectors > 0 else 0
            ...
            "phase4_specialized_connectors_audit": {
                ...
                "connector_details": {
                    "common_crawl": {
                        "enabled": True,
                        "result_summary": {
                            "total_results": result.common_crawl_result.total_results if result.common_crawl_result else 0,
                            "warc_records": len(result.common_crawl_result.warc_records) if result.common_crawl_result else 0
                        } if result.common_crawl_result else None
                    },
                    "memento_temporal": {
                        "enabled": True,
                        "result_summary": {
                            "snapshots": len(result.temporal_analysis_result.snapshots) if result.temporal_analysis_result else 0,
                            "temporal_diffs": len(result.temporal_analysis_result.temporal_diffs) if result.temporal_analysis_result else 0
                        } if result.temporal_analysis_result else None
                    },
                    "ahmia_tor": {
                        "enabled": True,
                        "result_summary": {
                            "legal_sources": len(result.ahmia_result.legal_sources) if result.ahmia_result else 0,
                            "filtered_count": result.ahmia_result.filtered_count if result.ahmia_result else 0
                        } if result.ahmia_result else None
                    },
                    "legal_apis": {
                        "enabled": True,
                        "result_summary": {
                            "court_documents": len(result.legal_apis_result.court_documents) if result.legal_apis_result else 0,
                            "sec_filings": len(result.legal_apis_result.sec_filings) if result.legal_apis_result else 0
                        } if result.legal_apis_result else None
                    }
                }
            }
        }
    ...
    async def _perform_differential_analysis(self, results_t1: List[Dict], results_t2: List[Dict]) -> Dict[str, Any]:
