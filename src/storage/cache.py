...
class SemanticResearchCache:
    ...
    def _create_cache_key(self, query: str, **kwargs) -> str:
        ...
        key_data = {"query": normalized_query, **{k: v for k, v in kwargs.items() if v is not None}}

        key_string = json.dumps(key_data, sort_keys=True)
        return hashlib.sha256(key_string.encode()).hexdigest()

    def _vectorize_query(self, query: str) -> np.ndarray | None:
            ...
            if query_vector is not None:
                ...
                for similar_key, similarity in similar_entries:
                    ...
                    if similar_result is not None:
                        ...
                        self.stats["retrieval_times"].append(retrieval_time)
                        logger.info(
                            f"Semantic cache hit (similarity: {similarity:.3f}) for query: {query[:50]}..."
                        )
    ...
    def _get_exact(self, cache_key: str) -> Any | None:
        ...
        with self.lock:
            try:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    # Delete from database
                    conn.execute(
                        "DELETE FROM cache_entries WHERE expiry_time < ?",
                        (current_time.isoformat(),),
                    )
        ...
        with self.lock:
            try:
                ...
                if self.stats["retrieval_times"]:
                    avg_retrieval_time = sum(self.stats["retrieval_times"]) / len(
                        self.stats["retrieval_times"]
                    )
