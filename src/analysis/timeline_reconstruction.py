...
class TimelineConflict:
    ...
    recommended_resolution: Optional[TimelineEvent]
...
class TimelineReconstructor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.temporal_tolerance = timedelta(days=config.get('timeline', {}).get('temporal_tolerance_days', 7))
    ...
    async def reconstruct_timeline(self, sources: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'methodology': {
                'extraction_methods': ['date_pattern_matching', 'nlp_temporal_extraction', 'metadata_analysis'],
                'merge_criteria': ['temporal_proximity', 'semantic_similarity', 'entity_overlap'],
                'conflict_resolution': ['source_reliability', 'evidence_weight', 'consensus_analysis']
            }
        }
    ...
    async def _extract_events_from_source(self, source: Dict[str, Any]) -> List[TimelineEvent]:
        ...
        try:
            ...
            date_events = await self._extract_events_by_date_patterns(content, source_id, source_reliability)
            events.extend(date_events)

            # Method 2: Temporal phrase extraction
            phrase_events = await self._extract_events_by_temporal_phrases(content, source_id, source_reliability)
            ...
            context_events = await self._extract_contextual_events(content, source_id, source_reliability)
    ...
    async def _extract_events_by_date_patterns(self, content: str, source_id: str, reliability: float) -> List[TimelineEvent]:
        ...
        for pattern in date_patterns:
            ...
            for match in matches:
                ...
                event = TimelineEvent(
                    ...
                    sources=[source_id],
                    confidence_score=reliability * self._calculate_extraction_confidence(context, date_str),
    ...
    async def _extract_events_by_temporal_phrases(self, content: str, source_id: str, reliability: float) -> List[TimelineEvent]:
    ...
    async def _merge_event_group(self, events: List[TimelineEvent]) -> TimelineEvent:
        ...
        weighted_timestamp = sum(
            e.timestamp.timestamp() * e.confidence_score for e in events
        ) / total_weight
    ...
    async def _detect_timeline_conflicts(self, events: List[TimelineEvent]) -> List[TimelineConflict]:
    ...
    async def _parse_flexible_date(self, date_str: str) -> Optional[datetime]:
