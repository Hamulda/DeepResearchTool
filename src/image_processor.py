...
class ImageProcessor:
    ...
    def _parse_gps_data(self, gps_dict: Dict) -> Optional[Dict[str, Any]]:
    ...
    def _detect_content_type(self, image: Image.Image) -> str:
        ...
        try:
            ...
            # Photo detection
            elif image.mode == "RGB" and min(width, height) > 200:
                return "photo"

            # Document/text detection (často vysoký a úzký)
            elif aspect_ratio < 0.8 or aspect_ratio > 2.5:
                return "document"

            # Icon/logo detection (malý a čtvercový)
            elif max(width, height) < 200 and 0.5 < aspect_ratio < 2.0:
                return "icon"
