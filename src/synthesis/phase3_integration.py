...
from ..verify.adversarial_verification import (
    AdversarialVerificationEngine,
...
class Phase3Integrator:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.enable_parallel_processing = self.integration_config.get(
        ...
        try:
            ...
            await asyncio.gather(
                (
                    ...
                    if hasattr(self.synthesis_engine, "initialize")
    ...
    async def process_synthesis_and_verification(
        ...
        try:
            ...
            synthesis_result = await self._step1_enhanced_synthesis(
            ...
            disagreement_coverage = await self._step2_counter_evidence_detection(
                synthesis_result, evidence_passages
            )
            counter_evidence_time = time.time() - counter_evidence_start

            self._log_processing_step(
                "counter_evidence_detection",
                {
            ...
            adversarial_verification = await self._step3_adversarial_verification(
                synthesis_result, evidence_passages
            )
            adversarial_time = time.time() - adversarial_start

            avg_robustness = (
                sum(r.overall_robustness for r in adversarial_verification)
                / len(adversarial_verification)
            ...
            await self._validate_final_quality_gates(
                final_quality_metrics, evidence_binding_quality
            )
    ...
    async def _step1_enhanced_synthesis(
        self, query: str, compressed_content: str, evidence_passages: list[dict[str, Any]]
    ) -> SynthesisResult:
        """STEP 1: Enhanced synthesis with evidence binding"""
    ...
    async def _step2_counter_evidence_detection(
        self, synthesis_result: SynthesisResult, evidence_passages: list[dict[str, Any]]
        ...
        return await self.counter_evidence_detector.detect_counter_evidence(
            claims, evidence_passages
        )

    async def _step3_adversarial_verification(
        self, synthesis_result: SynthesisResult, evidence_passages: list[dict[str, Any]]
    ) -> list[AdversarialVerificationResult]:
        """STEP 3: Adversarial verification"""
        ...
        return await self.adversarial_verifier.verify_claims_adversarially(
            claims_with_evidence, evidence_passages
        )

    async def _step4_update_synthesis_with_verification(
        self,
        synthesis_result: SynthesisResult,
        disagreement_coverage: DisagreementCoverage,
        adversarial_verification: list[AdversarialVerificationResult],
        ...
        disagreement_map = {
            analysis.claim_id: analysis for analysis in disagreement_coverage.detailed_analysis
        }
        ...
        for binding in synthesis_result.evidence_bindings:
            ...
            if claim_id in disagreement_map:
                ...
                binding.confidence_score = max(
                    0.1, binding.confidence_score - disagreement_analysis.confidence_adjustment
                )

                # Update verification notes
                binding.verification_notes += (
                    f" Disagreement analysis: {disagreement_analysis.analysis_notes}"
                )
            ...
            if claim_id in adversarial_map:
                ...
                binding.confidence_score = max(
                    0.1, binding.confidence_score - adversarial_result.confidence_adjustment
                )

                # Update verification notes
                binding.verification_notes += (
                    f" Adversarial verification: {adversarial_result.verification_notes}"
                )
        ...
        verified_citations = sum(
            len(
                [
                    s
                    for s in b.citation_slots
                    if s.verification_status in ["verified", "partially_verified"]
                ]
            )
            for b in synthesis_result.evidence_bindings
        )

        synthesis_result.verification_score = (
    ...
    def _calculate_final_quality_metrics(
        ...
        citation_coverage = (
            1.0
            if synthesis_result.citation_count >= len(synthesis_result.evidence_bindings) * 2
            else 0.5
        )
        ...
        if adversarial_verification:
            avg_robustness = sum(r.overall_robustness for r in adversarial_verification) / len(
                adversarial_verification
            )
        ...
        required_independence = (
            len(synthesis_result.evidence_bindings) * 2
        )  # 2 independent sources per claim
        actual_independence = synthesis_result.independent_sources
        metrics["citation_precision"] = (
            min(actual_independence / required_independence, 1.0)
            if required_independence > 0
            else 0
        )
        ...
        if synthesis_result.evidence_bindings:
            avg_evidence_strength = sum(
                b.evidence_strength for b in synthesis_result.evidence_bindings
            ) / len(synthesis_result.evidence_bindings)
    ...
    def _calculate_evidence_binding_quality(
        ...
        well_cited_claims = sum(
            1 for b in synthesis_result.evidence_bindings if len(b.citation_slots) >= 2
        )
        ...
        verified_bindings = sum(
            ...
            for b in synthesis_result.evidence_bindings
            if any(
                slot.verification_status in ["verified", "partially_verified"]
                for slot in b.citation_slots
            )
        )
        ...
        precise_citations = sum(
            len(
                [
                    slot
                    for slot in binding.citation_slots
                    if slot.char_start >= 0 and slot.char_end > slot.char_start
                ]
            )
            for binding in synthesis_result.evidence_bindings
        )
        total_citations = sum(
            len(binding.citation_slots) for binding in synthesis_result.evidence_bindings
        )
        metrics["char_offset_precision"] = (
            precise_citations / total_citations if total_citations > 0 else 0
        )

        # Evidence strength distribution
        strength_scores = [
            binding.evidence_strength for binding in synthesis_result.evidence_bindings
        ]
    ...
    async def _validate_final_quality_gates(
        self, final_quality_metrics: dict[str, float], evidence_binding_quality: dict[str, float]
    ):
        ...
        if failed_validations:
            ...
            for failure in failed_validations:
                logger.warning(
                    f"Quality issue: {failure['metric']} = {failure['actual']:.3f} (required: {failure['required']})"
                )
    ...
    def get_integration_report(self, result: Phase3ProcessingResult) -> dict[str, Any]:
        ...
        report = {
            ...
            "adversarial_verification_summary": {
                ...
                "average_robustness": (
                    sum(r.overall_robustness for r in result.adversarial_verification)
                    / len(result.adversarial_verification)
                    if result.adversarial_verification
                    else 0
                ),
                "total_challenges": sum(
                    len(r.challenges_generated) for r in result.adversarial_verification
                ),
            },
    ...
    def export_complete_audit_trail(self, result: Phase3ProcessingResult, output_path: str):
        ...
        disagreement_report = await self.counter_evidence_detector.generate_disagreement_report(
            result.disagreement_coverage
        )
        adversarial_report = await self.adversarial_verifier.generate_verification_report(
                        ...
                        "citations": [
                            {
                                ...
                                "source_text": (
                                    slot.source_text[:200] + "..."
                                    if len(slot.source_text) > 200
                                    else slot.source_text
                                ),
                            }
                            for slot in binding.citation_slots
                        ],
