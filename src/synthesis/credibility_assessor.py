...
class DomainReputationMetrics:
    ...
    domain_age: int | None  # ve dnech
    alexa_rank: int | None
    trustworthiness_score: float
    known_disinformation: bool
    fact_check_rating: str | None
...
class TemporalRelevanceMetrics:
    ...
    publication_age: int | None  # ve dnech
...
class CredibilityAssessor:
    ...
    async def _count_spelling_errors(self, content: str) -> int:
        ...
        for word in words:
            ...
            if len(clean_word) > 2:
                ...
                if re.search(r"(.)\1{3,}", clean_word) or re.search(r"[bcdfghjklmnpqrstvwxyz]{4,}", clean_word) or re.search(r"[aeiou]{4,}", clean_word):
                    error_count += 1
    ...
    async def _estimate_domain_age(self, domain: str) -> int | None:
        ...
        if any(tld in domain for tld in [".com", ".net"]):
    ...
    async def _classify_domain_category(self, domain: str) -> str:
        ...
        if any(keyword in domain for keyword in ["blog", "medium", "wordpress"]):
            return "blog"
        if any(keyword in domain for keyword in ["facebook", "twitter", "instagram", "social"]):
            return "social_media"
        if any(keyword in domain for keyword in ["forum", "reddit", "discussion"]):
            return "forum"
        return "general"
