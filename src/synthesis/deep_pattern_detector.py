...
class DeepPatternDetector:
    ...
    def _get_pattern_category(self, pattern_name: str) -> str:
        ...
        if any(x in pattern_name for x in ["onion", "i2p", "darknet"]):
            return "darknet"
        if any(x in pattern_name for x in ["gps", "coordinate", "location"]):
            return "geolocation"
        if any(x in pattern_name for x in ["email", "telegram", "discord", "communication"]):
            return "communication"
        if any(x in pattern_name for x in ["hash", "md5", "sha", "fingerprint"]):
            return "cryptographic_hashes"
        if any(x in pattern_name for x in ["ip", "mac", "domain", "url", "network"]):
            return "network"
        if any(x in pattern_name for x in ["passport", "credit", "ssn", "phone", "iban"]):
            return "personal_documents"
        if any(x in pattern_name for x in ["timestamp", "date", "time"]):
            return "temporal"
        if any(x in pattern_name for x in ["base64", "hex", "rot13", "encoding"]):
            return "encoding"
    ...
    async def _calculate_pattern_confidence(
        ...
        if pattern_name.startswith("ethereum"):
        ...
        if pattern_name == "email_standard":
            return await self._validate_email_format(matched_text)
        if pattern_name.endswith("_hash"):
