...
def validate_config(config: dict[str, Any]) -> None:
    ...
    required_sections = ["profiles", "qdrant", "retrieval", "compression", "claim_graph"]
    ...
    if features.get("hierarchical_retrieval") and not config.get("retrieval", {}).get(
        "hierarchical", {}
    ).get("enabled"):
...
def create_default_config() -> dict[str, Any]:
    ...
    default_config = {
        ...
        "qdrant": {"url": "http://localhost:6333", "embedding_model": "all-MiniLM-L6-v2"},
        "retrieval": {
            "hierarchical": {"enabled": True, "levels": 2},
            "rrf": {"k": 40},
        ...
        "claim_graph": {"enabled": True, "contradiction_detection": {"enabled": True}},
        "features": {
        ...
        "logging": {"level": "INFO", "file": "./logs/deep_research_tool.log"},
    }
