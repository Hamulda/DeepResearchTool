...
class InformationPriorityScorer:
    ...
    def __init__(self):
        ...
        self.keyword_weights = {
            'conspiracy_keywords': {
                'declassified': 0.9, 'classified': 0.8, 'secret': 0.7, 'cover-up': 0.85,
                'conspiracy': 0.6, 'hidden': 0.5, 'suppressed': 0.8, 'leaked': 0.9,
                'whistleblower': 0.95, 'revelation': 0.7, 'exposed': 0.8, 'confidential': 0.85
            },
            'research_keywords': {
                'study': 0.7, 'research': 0.8, 'analysis': 0.6, 'investigation': 0.85,
                'evidence': 0.9, 'data': 0.5, 'findings': 0.8, 'results': 0.6,
                'conclusion': 0.7, 'methodology': 0.6, 'peer-reviewed': 0.95, 'clinical trial': 0.9
            },
            'authority_keywords': {
                'government': 0.6, 'official': 0.7, 'agency': 0.6, 'department': 0.5,
                'military': 0.7, 'intelligence': 0.8, 'document': 0.6, 'report': 0.5,
                'fbi': 0.8, 'cia': 0.8, 'nsa': 0.8, 'pentagon': 0.7
            },
            'temporal_keywords': {
                'historical': 0.6, 'archive': 0.7, 'vintage': 0.5, 'original': 0.8,
                'primary source': 0.9, 'contemporary': 0.6, 'period': 0.4, 'era': 0.4,
                'decade': 0.4, 'recently declassified': 0.95, 'newly discovered': 0.9
            },
            'medical_keywords': {
                'peptide': 0.9, 'hormone': 0.8, 'clinical': 0.8, 'therapeutic': 0.8,
                'mechanism': 0.7, 'pharmacokinetics': 0.9, 'bioavailability': 0.8,
                'receptor': 0.7, 'pathway': 0.7, 'absorption': 0.6, 'metabolism': 0.7,
                'half-life': 0.6, 'dosage': 0.5, 'side effects': 0.6, 'contraindication': 0.7
            },
            'quality_indicators': {
                'peer-reviewed': 0.95, 'meta-analysis': 0.9, 'systematic review': 0.85,
                'randomized controlled trial': 0.9, 'double-blind': 0.8, 'placebo-controlled': 0.8,
                'longitudinal study': 0.7, 'cohort study': 0.7, 'case study': 0.5
            },
            'breakthrough_indicators': {
                'breakthrough': 0.95, 'revolutionary': 0.9, 'novel': 0.8, 'first-of-its-kind': 0.95,
                'unprecedented': 0.9, 'paradigm shift': 0.95, 'game-changing': 0.85,
                'groundbreaking': 0.9, 'innovative': 0.7, 'cutting-edge': 0.8
            },
            'controversy_indicators': {
                'controversial': 0.7, 'disputed': 0.8, 'debated': 0.6, 'contradictory': 0.75,
                'conflicting': 0.7, 'questioned': 0.6, 'challenged': 0.65, 'refuted': 0.8
            }
        }

        # Enhanced source credibility with more granular scoring
        self.source_credibility = {
            'arxiv.org': 0.9, 'pubmed.ncbi.nlm.nih.gov': 0.95, 'nature.com': 0.95,
            'science.org': 0.95, 'cell.com': 0.9, 'nejm.org': 0.95, 'thelancet.com': 0.9,
            'archive.org': 0.85, 'government': 0.8, 'academic': 0.85, '.edu': 0.85,
            'clinicaltrials.gov': 0.9, 'cochrane.org': 0.95, 'who.int': 0.9,
            'nih.gov': 0.9, 'fda.gov': 0.85, 'cdc.gov': 0.85,
            'news_mainstream': 0.7, 'news_alternative': 0.5, 'blog': 0.3, 'forum': 0.2,
            'social_media': 0.1, 'unknown': 0.4, 'peptide_database': 0.85, 'medical_database': 0.9
        }
        ...
        self.quality_patterns = {
            'high_quality': [
                r'\bpeer.?reviewed?\b', r'\bmeta.?analysis\b', r'\bsystematic\s+review\b',
                r'\brandomized\s+controlled\s+trial\b', r'\bdouble.?blind\b',
                r'\bplacebo.?controlled\b', r'\bmulticenter\b', r'\bprospective\b'
            ],
            'medium_quality': [
                r'\bcohort\s+study\b', r'\bcase.?control\b', r'\bobservational\s+study\b',
                r'\bcross.?sectional\b', r'\bpilot\s+study\b'
            ],
            'low_quality': [
                r'\bcase\s+report\b', r'\banecdotal\b', r'\bopinion\b',
                r'\beditorial\b', r'\bcommentary\b'
            ]
        }
        ...
        self.expertise_indicators = {
            'high_expertise': [
                r'\bprofessor\b', r'\bphd\b', r'\bmd\b', r'\bresearch\s+scientist\b',
                r'\bprincipal\s+investigator\b', r'\bdepartment\s+of\b', r'\buniversity\s+of\b'
            ],
            'institutional': [
                r'\bharvard\b', r'\bmit\b', r'\bstanford\b', r'\bmayo\s+clinic\b',
                r'\bjohns\s+hopkins\b', r'\bnih\b', r'\bwho\b', r'\bfda\b'
            ]
        }
        ...
        self.statistical_indicators = {
            ...
            'statistical_tests': [
                r'\bp\s*<\s*0\.05\b', r'\bp\s*<\s*0\.01\b', r'\bp\s*<\s*0\.001\b',
                r'\bconfidence\s+interval\b', r'\bci\s*[:=]\s*\d+%\b',
                r'\bt-test\b', r'\banova\b', r'\bregression\b'
            ]
        }
    ...
    def _score_uniqueness(self, content: str, metadata: Dict[str, Any]) -> float:
        ...
        unique_indicators = [
            'exclusive', 'unprecedented', 'never before', 'first time',
            'newly discovered', 'recently declassified', 'breaking'
        ]
        ...
        for pattern in date_patterns:
            ...
            if matches:
                try:
                    ...
                    elif '-' in content[content.find(match[0]):content.find(match[0]) + 20]:
                        # YYYY-MM-DD format
    ...
    def _classify_content_type(self, content: str, metadata: Dict[str, Any]) -> str:
        ...
        if any(word in content_lower for word in ['conspiracy', 'theory', 'cover-up', 'secret']):
            ...
            return 'research'
        elif any(word in content_lower for word in ['breaking', 'news', 'report', 'update']):
            return 'news'
        elif any(word in content_lower for word in ['archive', 'historical', 'vintage', 'original']):
            return 'archive'
    ...
    def _score_semantic_coherence(self, content: str) -> float:
        ...
        connector_count = sum(1 for sentence in valid_sentences
    ...
    def _score_statistical_rigor(self, content: str) -> float:
        ...
        effect_size_patterns = [
            r'\bcohen\'?s\s+d\b', r'\beffect\s+size\b', r'\bodds\s+ratio\b',
            r'\bhazard\s+ratio\b', r'\bretative\s+risk\b'
        ]
    ...
    def _score_controversy_indicators(self, content: str) -> float:
        ...
        opposition_patterns = [
            r'\bhowever\b', r'\bbut\b', r'\bcontrary\s+to\b', r'\bon\s+the\s+other\s+hand\b',
            r'\bnevertheless\b', r'\bdespite\b', r'\balthough\b'
        ]

        opposition_count = sum(1 for pattern in opposition_patterns
                             if re.search(pattern, content, re.IGNORECASE))
    ...
    def _score_breakthrough_indicators(self, content: str) -> float:
        ...
        novelty_patterns = [
            r'\bfirst\s+time\b', r'\bnever\s+before\b', r'\bpreviously\s+unknown\b',
            r'\bnew\s+discovery\b', r'\brecent\s+breakthrough\b'
        ]
    ...
    def _calculate_confidence_level(self, scores: Dict[str, float], content: str, metadata: Dict[str, Any]) -> float:
    ...
    def _adjust_weights_by_confidence(self, base_weights: Dict[str, float], confidence: float) -> Dict[str, float]:
    ...
    def _assess_reliability(self, scores: Dict[str, float], metadata: Dict[str, Any]) -> str:
        ...
        reliability_score = (scores['source_credibility'] * 0.4 +
