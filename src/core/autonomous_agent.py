...
class AutonomousResearchAgent:
    ...
    async def autonomous_research(self, initial_topic: str) -> Dict[str, Any]:
        ...
        while iteration < self.max_iterations and self._has_promising_leads():
            ...
            logger.info(f"Decision: {decision.action} targeting '{decision.target}' - {decision.reasoning}")
...
"""
            ...
            response = await self.orchestrator.ollama_agent._query_ollama_async(lead_extraction_prompt)
    ...
    async def _make_research_decision(self) -> Optional[ResearchDecision]:
        ...
        elif self.current_phase == ResearchPhase.CROSS_VALIDATION:
    ...
    async def _decide_initial_discovery(self, lead: ResearchLead) -> ResearchDecision:
        ...
        elif any(keyword in lead.context.lower() for keyword in ['cia', 'fbi', 'government', 'agency']):
            return ResearchDecision(
    ...
    async def _update_knowledge_graph(self, sources: List[Dict[str, Any]], query: str):
        ...
        try:
            ...
            entities_data = self.orchestrator.ollama_agent.extract_entities_and_relationships(combined_content)
    ...
    def _advance_research_phase(self):
        ...
        verified_leads = len([lead for lead in self.research_leads if lead.verification_status == "verified"])
    ...
    def _has_promising_leads(self) -> bool:
        ...
        unverified_leads = [lead for lead in self.research_leads if lead.verification_status == "unverified"]
    ...
    async def _synthesize_autonomous_findings(self, topic: str, sources: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            autonomous_metadata = {
                ...
                'verification_rate': len([l for l in self.research_leads if l.verification_status == "verified"]) / max(len(self.research_leads), 1),
                'knowledge_graph_entities': sum(len(kg.get('entities', [])) for kg in self.knowledge_graph.values()),
