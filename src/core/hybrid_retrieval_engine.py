...
from qdrant_client.models import Distance, VectorParams, PointStruct, Filter, FieldCondition, SearchRequest
...
class HierarchicalChunker:
    ...
    def _split_into_passages(self, section: HierarchicalChunk, doc_id: str) -> List[HierarchicalChunk]:
    ...
    def _split_into_sentences(self, passage: HierarchicalChunk, doc_id: str) -> List[HierarchicalChunk]:
...
class HybridRetrievalEngine:
    """Hybridní retrieval engine s hierarchickým vyhledáváním"""
    ...
    async def initialize(self):
        ...
        self.logger.info("Initializing hierarchical retrieval engine")
    ...
    async def _search_level(self, query_embedding: np.ndarray, collection: str,
                           top_k: int, filter_parent: str = None) -> List[SearchResult]:
        ...
        if filter_parent:
            search_filter = Filter(
                must=[
                    FieldCondition(key="parent_id", match={"value": filter_parent})
                ]
            )
    ...
    async def index_document(self, document: Dict[str, Any]):
        ...
        self.logger.info(f"Indexed document {document.get('id')} with {len(chunks)} hierarchical chunks")
