...
class DAGWorkflowOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_parallel_subqueries = self.workflow_config.get("phases", {}).get("retrieval", {}).get("parallel_subqueries", 4)
    ...
    async def execute_research_workflow(self,
        ...
        self.logger.info("Spouštím research workflow",
                        query=main_query,
        ...
        try:
            ...
            final_result = await self._compile_final_result(
                main_query, retrieval_result, reranking_result,
    ...
    async def _execute_retrieval_phase(self,
        ...
        for i, subquery in enumerate(expanded_queries):
            ...
            retrieval_tasks.append(
                self.retrieval_engine.search(subquery, max_results=max_docs//len(expanded_queries))
            )
    ...
    async def _execute_synthesis_phase(self,
                                     main_query: str,
                                     reranking_result: Dict[str, Any]) -> Dict[str, Any]:
        """Fáze 3: Synthesis s evidence binding"""
        ...
        synthesis_result = await self.synthesis_engine.synthesize_with_evidence(
            ...
            min_evidence_per_claim=self.workflow_config.get("phases", {}).get("synthesis", {}).get("min_evidence_per_claim", 2)
        )
    ...
    async def _execute_verification_phase(self, synthesis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        verification_result = await self.verification_engine.verify_claims(
            ...
            threshold=self.workflow_config.get("phases", {}).get("verification", {}).get("verification_threshold", 0.8)
        )
    ...
    async def _compile_final_result(self,
                                  main_query: str,
        ...
        return {
            ...
            "citation_summary": self._generate_citation_summary(synthesis_result, verification_result)
        }

    def _generate_citation_summary(self,
                                 synthesis_result: Dict[str, Any],
                                 verification_result: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for claim_id, evidence_list in evidence_bindings.items():
            for evidence in evidence_list:
                citation = {
                    ...
                    "verification_status": "verified" if claim_id in [c.get("id") for c in verification_result.get("verified_claims", [])] else "flagged"
                }
    ...
    async def _export_research_graph(self, final_result: Dict[str, Any]):
        ...
        if not self.config.get("observability", {}).get("tracing", {}).get("export_research_graph", False):
            return
        ...
        graph_formats = self.config.get("observability", {}).get("tracing", {}).get("graph_formats", ["json"])

        for format_type in graph_formats:
            if format_type == "json":
                graph_file = f"./data/graphs/research_graph_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def _export_metrics(self):
        ...
        metrics_file = f"./data/metrics/workflow_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
