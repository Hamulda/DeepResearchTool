...
from .ollama_agent import OllamaResearchAgent, AnalysisResult
...
class ResearchOrchestrator:
    ...
    async def advanced_research(self, topic: str, time_range: Tuple[datetime, datetime] = None,
                               analysis_type: str = 'advanced_research',
                               specialized_sources: List[str] = None) -> ResearchResults:
        """
        ...
        try:
            ...
            context = self.context_manager.build_context(
                unique_sources, cross_references, timeline
            )

            ai_analysis = await self.ollama_agent.analyze_research(
                topic, context, analysis_type
            )
            ...
            logger.info(f"Advanced research completed in {execution_time:.2f}s, found {len(unique_sources)} specialized sources")
            return results
        ...
        for scraper_name in query.sources:
            if scraper_name in self.scrapers:
                ...
                if scraper_name == 'osint_collector':
                    ...
                    task = asyncio.create_task(
                        self._safe_scrape(scraper, query.topic, query.time_range),
                        name=scraper_name
                    )
                tasks.append(task)
    ...
    async def _safe_osint_collection(self, osint_collector, topic: str, time_range: Optional[Tuple[datetime, datetime]]) -> List[Dict[str, Any]]:
    ...
    def _score_specialized_sources(self, sources: List[Dict[str, Any]],
        ...
        logger.info(f"Filtered {len(scored_sources)} high-priority specialized sources from {len(sources)} total")
    ...
    def _analyze_specialized_source_breakdown(self, sources: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for source in sources:
            ...
            if source_type in ['cia_declassified']:
                specialized_breakdown['government_declassified'] = specialized_breakdown.get('government_declassified', 0) + 1
            elif source_type in ['base_academic']:
                specialized_breakdown['deep_web_academic'] = specialized_breakdown.get('deep_web_academic', 0) + 1
            elif source_type in ['ipfs_content', 'bittorrent_dht']:
                specialized_breakdown['p2p_networks'] = specialized_breakdown.get('p2p_networks', 0) + 1
            elif source_type in ['osint_intelligence']:
                specialized_breakdown['osint_intelligence'] = specialized_breakdown.get('osint_intelligence', 0) + 1
    ...
    async def deep_research(self, topic: str, time_range: Tuple[datetime, datetime] = None,
                           analysis_type: str = 'general_research') -> ResearchResults:
        """
        ...
        try:
            ...
            logger.info(f"Deep research completed in {execution_time:.2f}s, found {len(unique_sources)} sources")
            return results
    ...
    def _score_and_filter_sources(self, sources: List[Dict[str, Any]],
                                 query: ResearchQuery) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Filtered {len(scored_sources)} high-priority sources from {len(sources)} total")
    ...
    async def _safe_scrape(self, scraper, topic: str, time_range: Optional[Tuple[datetime, datetime]]) -> List[Dict[str, Any]]:
