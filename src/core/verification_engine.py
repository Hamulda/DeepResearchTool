...
class VerificationEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.verification_config = config.get("workflow", {}).get("phases", {}).get("verification", {})
    ...
    async def initialize(self):
        ...
        self.verification_model = self.m1_config.get("ollama", {}).get("models", {}).get("verification", "qwen2.5:7b")

        self.logger.info("Verification engine inicializován",
    ...
    async def verify_claims(self,
        ...
        self.logger.info("Spouštím verifikaci claims",
...
"""
        ...
        try:
            ...
            self.logger.error("Chyba při verifikaci claim",
                            claim_id=claim_id,
                            error=str(e))
    ...
    def _create_fallback_verification(self,
                                    claim_id: str,
                                    claim_text: str,
                                    threshold: float) -> VerificationResult:
    ...
    def _create_reasoning_summary(self,
        ...
        for result in verification_results:
            summary_item = {
                ...
                "key_reasoning": result.reasoning[:200] + "..." if len(result.reasoning) > 200 else result.reasoning,
    ...
    async def cross_validate_claims(self,
                                  claims: List[Dict[str, Any]],
                                  evidence_bindings: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "cross_validation_summary": {
                "total_claims": len(claims),
                "conflicting_pairs": len([r for r in claim_relationships if r["relationship"] == "conflicting"]),
                "supporting_pairs": len([r for r in claim_relationships if r["relationship"] == "supporting"]),
                "consistency_score": self._calculate_consistency_score(consistency_issues, len(claims))
            }
        }

    async def _analyze_claim_relationships(self,
...
"""
    ...
    async def _identify_consistency_issues(self,
                                         claims: List[Dict[str, Any]],
    ...
    def _recommend_conflict_resolution(self,
                                     claim1: Dict[str, Any],
                                     claim2: Dict[str, Any]) -> str:
    ...
    async def _generate_resolution_recommendations(self,
        ...
        if high_severity_issues:
            recommendations.append(f"Kritické: {len(high_severity_issues)} konfliktních claims vyžaduje okamžité řešení")

            for issue in high_severity_issues[:3]:  # Top 3 kritické issues
                recommendations.append(f"- {issue.get('recommended_action', 'Vyžaduje manuální kontrolu')}")

        medium_severity_issues = [i for i in issues if i.get("severity") == "medium"]
        if medium_severity_issues:
            recommendations.append(f"Střední priorita: {len(medium_severity_issues)} claims vyžaduje dodatečnou verifikaci")

        return recommendations

    def _calculate_consistency_score(self, issues: List[Dict[str, Any]], total_claims: int) -> float:
