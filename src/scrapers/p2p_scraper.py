...
class IPFSScraper:
    ...
    async def _fetch_ipfs_content(self, ipfs_hash: str) -> str | None:
                    ...
                    results.append(
                        {
                            ...
                            "source_url": self.ipfs_gateways[0] + ipfs_hash,
                            "date": datetime.now(),
                            "metadata": {"ipfs_hash": ipfs_hash, "search_result": True},
                        }
                    )
...
class BitTorrentDHTScraper:
    ...
    async def search_async(
        ...
        logger.info(
            f"BitTorrent DHT search for '{topic}' found {len(filtered_results)} document torrents"
        )
    ...
    def _parse_dht_response(self, html_content: str, topic: str) -> list[dict[str, Any]]:
        ...
        for i, (hash_val, name) in enumerate(zip(magnet_links, names, strict=False)):
...
class P2PNetworkScraper:
    ...
    async def search_async(
        ...
        logger.info(
            f"P2P network search for '{topic}' found {len(deduplicated_results)} total results"
        )
