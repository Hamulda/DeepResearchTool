...
class PeptideResearchScraper:
    ...
    def __init__(self, rate_limit: float = 0.6):
        ...
        self.peptide_database = {
            'growth_peptides': {
                'cjc-1295': {
                    ...
                    'effects': ['increased GH', 'increased IGF-1', 'improved sleep', 'muscle growth'],
                ...
                'mk-677': {
                    ...
                    'type': 'Growth Hormone Secretagogue',
                    'mechanism': 'Ghrelin receptor agonist',
                    'effects': ['increased GH', 'increased IGF-1', 'increased appetite', 'improved sleep'],
                    'absorption': 'oral bioavailability',
                    'half_life': '24 hours',
                    'related_terms': ['ibutamoren', 'growth hormone secretagogue', 'ghrelin mimetic']
                }
            },
            'healing_peptides': {
                'bpc-157': {
                    ...
                    'effects': ['tendon healing', 'gut protection', 'wound healing', 'neuroprotection'],
                    'absorption': 'oral, subcutaneous, intramuscular',
                    'half_life': 'short (stabilized forms available)',
                    'related_terms': ['gastric juice peptide', 'pentadecapeptide', 'cytoprotective']
                },
                'tb-500': {
            ...
            'cosmetic_peptides': {
                ...
                'pt-141': {
                    ...
                    'half_life': '2-3 hours',
                    'related_terms': ['bremelanotide', 'melanocortin', 'sexual dysfunction', 'libido']
                }
            }
        }
        ...
        self.hormone_pathways = {
            'growth_hormone_axis': [
                'hypothalamus', 'GHRH', 'somatostatin', 'pituitary', 'growth hormone',
                'liver', 'IGF-1', 'IGF-BP', 'muscle', 'bone', 'fat'
            ],
            'hpa_axis': [
                'hypothalamus', 'CRH', 'pituitary', 'ACTH', 'adrenal', 'cortisol',
                'stress response', 'immune system', 'metabolism'
            ],
            'reproductive_axis': [
                'hypothalamus', 'GnRH', 'pituitary', 'LH', 'FSH', 'gonads',
    ...
    def _enhance_peptide_knowledge(self, topic: str) -> List[str]:
        ...
        for category, peptides in self.peptide_database.items():
            for peptide_name, info in peptides.items():
                if (peptide_name in topic_lower or
                    info['full_name'].lower() in topic_lower or
                    any(term.lower() in topic_lower for term in info['related_terms'])):
        ...
        biochem_terms = [
            'receptor binding', 'bioavailability', 'half life',
            'side effects', 'dosage', 'clinical study'
        ]
    ...
    async def _search_peptide_source(self, session: aiohttp.ClientSession, source_id: str,
    ...
    async def _search_specialized_peptide_db(self, session: aiohttp.ClientSession,
                                           source_config: Dict[str, Any], query: str) -> List[Dict[str, Any]]:
    ...
    def _parse_peptide_api_response(self, data: Dict[str, Any], source_config: Dict[str, Any>,
        ...
        try:
            ...
            for item in peptide_data[:20]:  # Limit results
                if isinstance(item, dict):
                    result = {
                        'title': item.get('name', item.get('title', f"Peptide from {source_config['name']}")),
    ...
    def _score_peptide_priority(self, result: Dict[str, Any], query: str) -> float:
        ...
        if result.get('activity'):
            ...
            if any(term in activity_lower for term in ['therapeutic', 'clinical', 'approved', 'trial']):
                priority_score += 0.2
    ...
    def _parse_peptide_html(self, soup: BeautifulSoup, source_config: Dict[str, Any], query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            entries = (
                soup.find_all('div', class_=re.compile(r'(peptide|entry|result)', re.I)) or
                soup.find_all('tr', class_=re.compile(r'(peptide|entry|result)', re.I)) or
            ...
            for entry in entries[:15]:  # Limit results
                title_elem = (
                    ...
                    entry.find('a', href=True) or entry.find(class_=re.compile(r'title', re.I))
                )

                title = title_elem.get_text(strip=True) if title_elem else f"Peptide from {source_config['name']}"
    ...
    def _extract_date_from_item(self, item: Dict[str, Any]) -> Optional[datetime]:
    ...
    def _extract_date_from_html(self, entry) -> Optional[datetime]:
    ...
    async def _search_peptide_atlas(self, session: aiohttp.ClientSession,
                                   source_config: Dict[str, Any], query: str) -> List[Dict[str, Any]]:
        """Search PeptideAtlas specifically"""
        return await self._search_specialized_peptide_db(session, source_config, query)

    async def _search_biopep(self, session: aiohttp.ClientSession,
                            source_config: Dict[str, Any], query: str) -> List[Dict[str, Any]]:
        """Search BIOPEP-UWM specifically"""
        return await self._search_specialized_peptide_db(session, source_config, query)

    async def _search_generic_peptide_source(self, session: aiohttp.ClientSession,
                                           source_config: Dict[str, Any], query: str) -> List[Dict[str, Any]]:
    ...
    def _filter_peptide_results(self, results: List[Dict[str, Any]], topic: str) -> List[Dict[str, Any]]:
    ...
    def _classify_research_domain(self, result: Dict[str, Any]) -> str:
        ...
        if any(term in combined_text for term in ['growth hormone', 'gh', 'igf-1', 'muscle', 'anti-aging']):
            return 'growth_hormone_research'
        elif any(term in combined_text for term in ['healing', 'repair', 'wound', 'regeneration']):
            return 'tissue_repair'
        elif any(term in combined_text for term in ['cancer', 'tumor', 'oncology', 'anticancer']):
            return 'oncology'
        elif any(term in combined_text for term in ['antimicrobial', 'antibiotic', 'infection']):
            return 'antimicrobial'
        elif any(term in combined_text for term in ['cosmetic', 'skin', 'tanning', 'beauty']):
            return 'cosmetic'
        elif any(term in combined_text for term in ['hormone', 'endocrine', 'oxytocin', 'vasopressin']):
    ...
    def _assess_clinical_relevance(self, result: Dict[str, Any]) -> str:
        ...
        if any(term in content for term in ['approved', 'fda approved', 'clinical trial', 'phase iii']):
            return 'high'
        elif any(term in content for term in ['clinical', 'therapeutic', 'treatment', 'therapy']):
            return 'medium'
        elif any(term in content for term in ['research', 'study', 'investigation']):
            return 'research'
        else:
            return 'experimental'
