...
class MedicalResearchScraper:
    ...
    async def _search_medical_source(
        self,
            ...
            async with session.get(search_url) as response:
                if response.status == 200:
                    ...
                    if ids:
                        ...
                        fetch_url = source_config["api_url"] + source_config[
                            "fetch_endpoint"
                        ].format(
                ...
                if response.status == 200:
                    ...
                    for protein in data.get("results", []):
                        try:
                            ...
                            gene_name = (
                                protein.get("genes", [{}])[0].get("geneName", {}).get("value", "")
                    ...
                    for molecule in data.get("molecules", []):
                        try:
                            molecule_name = molecule.get(
                                "pref_name", molecule.get("molecule_chembl_id", "Unknown")
                            )
    ...
    async def _search_generic_medical(
        self, session: aiohttp.ClientSession, source_config: dict[str, Any], query: str
    ) -> list[dict[str, Any]]:
        ...
        for element in peptide_elements:
            try:
                ...
                if len(text) > 20 and any(
                    term in text.lower()
                    for term in ["peptide", "amino acid", "sequence", "bioactive"]
                ):
            ...
            try:
                ...
                if (
                    ...
                    and any(
                        ...
                        for term in ["pathway", "reaction", "enzyme", "protein"]
                    )
                ):
    ...
    def _parse_generic_medical_results(
        self, soup: BeautifulSoup, source_config: dict[str, Any], query: str
    ) -> list[dict[str, Any]]:
    ...
    async def search_specific_database(
        self, database_name: str, topic: str
    ) -> list[dict[str, Any]]:
