...
class SearchQuery:
    ...
    access_type: Optional[str] = None  # open_access, any
...
class BASESearchEngine:
    ...
    async def search_comprehensive(self,
                                 ...
                                 include_full_text: bool = True) -> List[AcademicDocument]:
        ...
        for search_string in search_strategies:
            batch_docs = await self._execute_search(
                search_string,
                max_results // len(search_strategies),
                include_full_text
            )
    ...
    async def _execute_search(self,
                            query_string: str,
                            max_results: int,
                            include_full_text: bool) -> List[AcademicDocument]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.base_url + '/cgi-bin/BaseHttpSearchInterface.fcgi',
            ...
            logger.error(f"Error executing BASE search: {str(e)}")
            # Try fallback method
            documents = await self._fallback_web_search(query_string, max_results)
    ...
    def _parse_search_results(self, data: Dict[str, Any], include_full_text: bool) -> List[AcademicDocument]:
    ...
    def _create_document_from_json(self, doc_data: Dict[str, Any], include_full_text: bool) -> Optional[AcademicDocument]:
        """Create AcademicDocument from JSON data"""
        try:
            # Extract basic information
            title = doc_data.get('dctitle', ['Unknown Title'])[0] if isinstance(doc_data.get('dctitle'), list) else doc_data.get('dctitle', 'Unknown Title')

            authors = doc_data.get('dccreator', [])
            if isinstance(authors, str):
                authors = [authors]

            abstract = doc_data.get('dcDescription', [''])[0] if isinstance(doc_data.get('dcDescription'), list) else doc_data.get('dcDescription', '')

            # Parse publication date
            pub_date = None
            if 'dcdate' in doc_data:
                date_str = doc_data['dcdate'][0] if isinstance(doc_data['dcdate'], list) else doc_data['dcdate']
                pub_date = self._parse_date(date_str)

            # Extract metadata
            publisher = doc_data.get('dcpublisher', ['Unknown'])[0] if isinstance(doc_data.get('dcpublisher'), list) else doc_data.get('dcpublisher', 'Unknown')
            source_repo = doc_data.get('collname', ['Unknown'])[0] if isinstance(doc_data.get('collname'), list) else doc_data.get('collname', 'Unknown')
            doc_type = doc_data.get('dctype', ['Unknown'])[0] if isinstance(doc_data.get('dctype'), list) else doc_data.get('dctype', 'Unknown')
            ...
            # URLs
            url = doc_data.get('dclink', [''])[0] if isinstance(doc_data.get('dclink'), list) else doc_data.get('dclink', '')
            doi = doc_data.get('dcdoi', [''])[0] if isinstance(doc_data.get('dcdoi'), list) else doc_data.get('dcdoi', None)
            ...
            logger.error(f"Error creating document from JSON: {str(e)}")
            return None

    def _create_document_from_html(self, entry) -> Optional[AcademicDocument]:
        ...
        try:
            ...
            logger.error(f"Error creating document from HTML: {str(e)}")
            return None

    def _extract_from_metadata(self, text: str, patterns: List[str]) -> Optional[str]:
    ...
    def _determine_access_rights(self, document: AcademicDocument) -> str:
        ...
        if 'open access' in document.document_type.lower():
            return 'open_access'
        elif any(oa_indicator in document.url.lower() for oa_indicator in ['arxiv', 'plos', 'doaj']):
            return 'open_access'
        elif any(sub_indicator in document.publisher.lower() for sub_indicator in ['elsevier', 'springer', 'wiley']):
            return 'subscription'
        else:
            return 'unknown'
    ...
    def _assess_complexity(self, document: AcademicDocument) -> float:
        ...
        complexity_indicators = [
            'algorithm', 'mathematical', 'theoretical', 'quantum', 'neural',
            'optimization', 'statistical', 'computational', 'methodology'
        ]
    ...
    def _assess_practical_relevance(self, document: AcademicDocument) -> float:
        ...
        practical_keywords = [
            'application', 'implementation', 'case study', 'real-world',
            'practical', 'empirical', 'experiment', 'evaluation'
        ]
    ...
    def create_research_dashboard(self, documents: List[AcademicDocument]) -> Dict[str, Any]:
        ...
        dashboard = {
            'overview': {
                ...
                'open_access_percentage': len([d for d in documents if d.access_rights == 'open_access']) / len(documents) * 100,
    ...
    def _generate_recommendations(self, documents: List[AcademicDocument]) -> List[str]:
        ...
        if len(high_quality_docs) > 5:
            recommendations.append(f"Focus on {len(high_quality_docs)} high-quality documents (score > 0.8)")
        ...
        if len(open_access_docs) > len(documents) * 0.3:
            recommendations.append(f"Good open access coverage: {len(open_access_docs)} documents freely available")

        # Recent research recommendations
        recent_docs = [d for d in documents if d.publication_date and
                      (datetime.now() - d.publication_date).days < 365 * 2]
        if len(recent_docs) > 10:
            recommendations.append(f"Strong recent research base: {len(recent_docs)} documents from last 2 years")

        # Interdisciplinary recommendations
        interdisciplinary_docs = [d for d in documents if d.metadata.get('interdisciplinary_score', 0) > 0.5]
        if len(interdisciplinary_docs) > 5:
            recommendations.append(f"Consider interdisciplinary approaches: {len(interdisciplinary_docs)} cross-domain documents found")
