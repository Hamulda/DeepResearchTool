...
class BASEScraper:
    ...
    def _create_search_strategies(self, topic: str) -> list[dict[str, Any]]:
        ...
        strategies.append(
            {"name": "direct_search", "query": topic, "filters": {}, "boost": "relevance"}
        )
        ...
        if any(
            term in topic_lower for term in ["history", "historical", "cold war", "vietnam", "wwii"]
        ):
            strategies.append(
                {
                    "name": "historical",
    ...
    async def _execute_base_search(
        self, strategy: dict[str, Any], time_range: tuple[datetime, datetime] | None = None
    ) -> list[dict[str, Any]]:
    ...
    def _parse_base_document(
        self, doc: dict[str, Any], strategy: dict[str, Any]
    ) -> dict[str, Any] | None:
    ...
    def _extract_document_date(self, doc: dict[str, Any]) -> datetime | None:
    ...
    def _deduplicate_results(self, results: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for result in results:
            ...
            identifier = (
                metadata.get("base_id", "")
                or metadata.get("identifier", "")
                or metadata.get("publication_info", {}).get("doi", "")
                or result.get("title", "")
            )
    ...
    def _is_recent_publication(self, doc_date: datetime | None) -> bool:
