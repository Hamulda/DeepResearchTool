...
class RegressionTestSuite:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.baseline_path = Path(
            config.get("regression", {}).get("baseline_path", "data/baselines")
        )
        self.results_path = Path(
            config.get("regression", {}).get("results_path", "docs/regression_results")
        )
    ...
    async def run_regression_tests(
        self, test_subset: list[str] | None = None
        ...
        for test_case in test_cases_to_run:
            ...
            try:
                ...
                status_emoji = (
                    ...
                    else "❌" if result.overall_status == "failed" else "⚠️"
                )
                print(f"{status_emoji} {test_case.domain}: {result.overall_status}")

            except Exception as e:
                print(f"💥 Chyba při testování {test_case.domain}: {e!s}")
                # Create failed result
                failed_result = RegressionTestResult(
                    domain=test_case.domain,
                    test_case=test_case,
                    evaluation_result=EvaluationResult(
                        query=test_case.query, timestamp=datetime.now()
                    ),
    ...
    async def _run_single_domain_test(self, test_case: DomainTestCase) -> RegressionTestResult:
        ...
        baseline_comparison = await self._compare_against_baseline(
            test_case.domain, evaluation_result
        )
        ...
        return RegressionTestResult(
            ...
            performance_metrics={
                ...
                "tokens_per_second": processing_metrics["token_budget_used"]
                / (processing_metrics["total_latency_ms"] / 1000),
    ...
    ) -> tuple[list[ClaimWithEvidence], list[dict[str, Any]]]:
        ...
        for i in range(test_case.expected_claim_count):
            ...
            for j in range(test_case.expected_evidence_per_claim):
                evidence = EvidenceItem(
                    ...
                    char_offset_end=100,
                    source_type=(
                        "primary_literature" if np.random.random() > 0.3 else "secondary_source"
                    ),
    ...
    async def _compare_against_baseline(
        self, domain: str, evaluation_result: EvaluationResult
    ) -> dict[str, float]:
        ...
        try:
            ...
            print(f"⚠️ Chyba při čtení baseline pro {domain}: {e!s}")
            return {}
    ...
    def _check_threshold_compliance(
        self, test_case: DomainTestCase, evaluation_result: EvaluationResult
    ) -> dict[str, bool]:
    ...
    def _determine_test_status(
        self, passed_thresholds: dict[str, bool], baseline_comparison: dict[str, float]
    ) -> str:
        ...
        if threshold_pass_rate >= 0.8:  # 80% thresholds must pass
            ...
            significant_degradations = sum(
                1
                for change in baseline_comparison.values()
                if change < -5.0  # More than 5% degradation
            )

            if significant_degradations == 0:
                return "passed"
            if significant_degradations <= 1:
                return "degraded"
            return "failed"
        return "failed"
        ...
        for result in failed_tests:
            critical_failure = {
                ...
                "baseline_degradations": [
                    k
                    for k, v in result.baseline_comparison.items()
    ...
    async def export_regression_results(
        self, results: list[RegressionTestResult], output_path: str | None = None
    ):
        """Exportuje regression results"""
        if output_path is None:
            output_path = (
                self.results_path
                / f"regression_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
