...
class RAGEvaluator:
    ...
    async def evaluate_context_precision(self, query: str, retrieved_contexts: List[str]) -> float:
        """
        ...
        try:
            ...
            relevant_count = response.lower().count("relevant") - response.lower().count(
                "not_relevant"
            )
    ...
    async def evaluate_context_recall(
        self, query: str, retrieved_contexts: List[str], expected_sources: List[str]
    ) -> float:
        """
        ...
        try:
            response = await self.llm_client.generate(prompt=prompt, temperature=0.1, max_tokens=10)
    ...
    async def evaluate_answer_relevance(self, query: str, answer: str) -> float:
        """
...
class EvaluationPipeline:
    ...
    async def evaluate_single_query(self, golden_item: GoldenDatasetItem) -> EvaluationResult:
        ...
        try:
            ...
            result = await self.research_agent.run(
                query=golden_item.query, session_id=f"eval_{golden_item.id}"
            )
    ...
    async def run_full_evaluation(self) -> Dict[str, Any]:
        ...
        return {"results": results, "metrics": metrics, "results_path": results_path}
    ...
    def _calculate_aggregate_metrics(self, results: List[EvaluationResult]) -> Dict[str, float]:
        ...
        metrics = {
            "avg_context_precision": sum(r.context_precision for r in valid_results)
            / len(valid_results),
            ...
            "avg_answer_relevance": sum(r.answer_relevance for r in valid_results)
            / len(valid_results),
            "avg_answer_correctness": sum(r.answer_correctness for r in valid_results)
            / len(valid_results),
    ...
    def _save_results(self, results: List[EvaluationResult], metrics: Dict[str, float], path: str):
    ...
    def check_regression_thresholds(self, metrics: Dict[str, float]) -> bool:
        ...
        for metric, threshold in thresholds.items():
            if metrics.get(metric, 0) < threshold:
                logger.error(
                    f"Regression detected: {metric} = {metrics.get(metric):.3f} < {threshold}"
                )
                return False
