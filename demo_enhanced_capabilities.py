...
class EnhancedResearchDemo:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            'research_config': {
                ...
                'osint': {
                    'professional_tools': {
                        'social_intelligence': {'enabled': True}
                    }
                },
                'security': {
    ...
    async def demo_cross_source_correlation(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            correlation_analysis = await self.specialized_analyzer.perform_cross_source_correlation(mock_sources)
            ...
            if mock_sources:
                ...
                bias_result = await self.ai_authenticator.detect_bias_and_propaganda(mock_sources[0])
    ...
    async def demo_comprehensive_research(self, query: str) -> Dict[str, Any]:
        ...
        comprehensive_results['research_modules']['declassified'] = await self.demo_declassified_documents_research(query)

        logger.info("Running historical archives research...")
        comprehensive_results['research_modules']['historical'] = await self.demo_historical_archives_research(query)
        ...
        comprehensive_results['research_modules']['correlation'] = await self.demo_cross_source_correlation(query)
        ...
        if avg_confidence > 0.7:
            recommendations.append("High confidence in findings - results are reliable for analysis")
        ...
        if any('error' in module for module in comprehensive_results['research_modules'].values()):
            recommendations.append("Some modules encountered errors - check configuration and API access")
        ...
        try:
            ...
            for module_name, module_results in results['research_modules'].items():
                ...
                if 'error' in module_results:
                    ...
                    if 'intelligence_results' in module_results:
                        print(f"  🔍 Intelligence results: {len(module_results['intelligence_results'])}")
